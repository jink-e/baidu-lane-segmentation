...
def train_epoch(net, epoch, dataLoader, optimizer, trainF, config):
    ...
    for batch_item in dataprocess:
        ...
        out = net(image)
        # 计算交叉熵loss
        mask_loss = MySoftmaxCrossEntropyLoss(nbclasses=config.NUM_CLASSES)(out, mask.long())
...
def main():
    ...
    if lane_config.PRETRAIN:  # 加载预训练权重
        print('Load pretrain weights: ', lane_config.PRETRAINED_WEIGHTS)
        net.load_state_dict(torch.load(lane_config.PRETRAINED_WEIGHTS, map_location='cuda:0'))
    if torch.cuda.is_available():
